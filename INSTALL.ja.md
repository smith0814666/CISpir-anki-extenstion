...
1. `github.com/smith0814666/CISpir-anki-extenstion` から ZIP をダウンロード
2. 解凍する
3. `chrome://extensions/` を開く → **デベロッパーモード** をオン
4. **パッケージ化されていない拡張機能を読み込む** → `dist` フォルダを選択

**方法C — ソースからビルド:** `git clone` → `npm install` → `npm run build` → `dist` を読み込む。
...
- **API キーなしでも使える?** 字幕・音声・保存は可。翻訳・文法・要約は不可。
